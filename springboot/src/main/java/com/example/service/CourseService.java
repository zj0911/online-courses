...
public class CourseService {
    ...
    public void add(Course course) {
        if(ObjectUtil.isNotEmpty(course.getRecommend()) && RecommendEnum.YES.equals(course.getRecommend())){
            //校验
            Course recommend = courseMapper.getRecommend(course.getType());
            if(ObjectUtil.isNotEmpty(recommend)){ //已经存在推荐
    ...
    public void updateById(Course course) {
        if(ObjectUtil.isNotEmpty(course.getRecommend()) && RecommendEnum.YES.equals(course.getRecommend())){
            //校验
            Course recommend = courseMapper.getRecommend(course.getType());
            if(ObjectUtil.isNotEmpty(recommend) && recommend.getId().equals(course.getId())){ //已经存在推荐
                throw new CustomException(ResultCodeEnum.RECOMMEND_ALREADY_ERROR);
            }
        }
